...
const App: React.FC = () => {
  ...
  // Teams State - Initialized from auth data
  ...
  const handleProfileUpdateInTeam = (name: string, email: string) => {
    setTeamMembers(prev => prev.map(member => {
      if (member.role === 'Owner') {
        ...
        return { ...member, name, email, initial: initials };
      }
      return member;
    }));
  };
  ...
  const handleLogin = (userData: { name: string; email: string; role: string }) => {
    ...
    handleProfileUpdateInTeam(userData.name, userData.email);
      ...
      <div className="flex-1 w-full px-4 pt-8 pb-32">
        {isScanning ? (
          ...
          <SettingsPage 
            ...
            onProfileUpdate={handleProfileUpdateInTeam}
